...
test('can castle', () => {
  const game = new Game(
    'rnbqkbnr/1ppppppp/8/8/p5P1/7N/PPPPPPBP/RNBQK2R w KQkq - 0 4'
  );
...
test('cannot castle while in check', () => {
  const game = new Game(
    'r1bqkbnr/1ppppppp/6P1/p7/8/7N/PPnPPPBP/RNBQK2R w KQkq - 0 6'
  );
...
test('cannot castle through line of check', () => {
  ...
  const game = new Game(
    'rn1qkbnr/p2ppppp/bpp5/8/4P3/5B1N/PPPP1PPP/RNBQK2R b KQkq - 3 4'
  );
  expect(game.makeMove('e1', 'g1')).toBeFalsy();
});
...
test('can en passant', () => {
  let game = new Game(
    'rnbqkbnr/1p1p1p1p/p5p1/1Pp1pP2/8/8/P1PPP1PP/RNBQKBNR w KQkq e6 0 5'
  );
